...
void DifferentialDriveController_init(DifferentialDriveController* ctr,
				      ...
				      JointInfo* status_left,
				      JointInfo* status_right,
				      JointControl* control_left,
				      JointControl* control_right,
				      float dt,
				      float joints_dt
				 ){
  ctr->params=params;
  ctr->status=status;
  ctr->control=control;
  ctr->status_left=status_left;
  ctr->control_left=control_left;
  ctr->status_right=status_right;
  ctr->control_right=control_right;
  ctr->dt=dt;
  ctr->idt=1./dt;
